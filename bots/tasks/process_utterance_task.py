...
from bots.utils import pcm_to_mp3, pcm_to_wav
...
def process_utterance(self, utterance_id):
    ...
    if utterance.transcription is None:
        ...
        elif recording.transcription_provider == TranscriptionProviders.DI_WHISPER_V3_TURBO:
            print("WHISPER TURBO")
            utterance.transcription = get_transcription_via_di_whisper_v3(utterance)
        elif recording.transcription_provider == TranscriptionProviders.DI_WHISPER_V3:
            print("WHISPER")
            utterance.transcription = get_transcription_via_di_whisper_v3(utterance)
        else:
...
def get_transcription_via_openai(utterance):
    ...
    options = {"file": ("file.mp3", payload_mp3, "audio/mpeg"), "model": (None, recording.bot.openai_transcription_model())}
    if recording.bot.openai_transcription_prompt():
        options["prompt"] = (None, recording.bot.openai_transcription_prompt())
    response = requests.post(url, headers=headers, files=options)
...
def get_transcription_via_di_whisper_v3(utterance):
    recording = utterance.recording
    # Convert PCM audio to MP3
    payload_wav = pcm_to_wav(utterance.audio_blob.tobytes(), sample_rate=utterance.sample_rate)

    client = Groq(api_key="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")
    transcription = client.audio.transcriptions.create(
        file=("file.wav", payload_wav),
    ...
    logger.info("Whipser transcription completed successfully")
