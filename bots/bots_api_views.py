...
from .serializers import (
    ...
    RecordingSerializer,
    SpeechSerializer,
    TranscriptUtteranceSerializer, BotListSerializer,
)
...
class BotListView(APIView):
    ...
    def get(self, request):
        try:
            bots = Bot.objects.filter(project=request.auth.project)
            return Response(BotListSerializer(bots).data)

        except Bot.DoesNotExist:
            return Response({"error": "Bot not found"}, status=status.HTTP_404_NOT_FOUND)


class BotListActiveView(APIView):
    authentication_classes = [ApiKeyAuthentication]
    ...
    def get(self, request):
        try:
            bots = Bot.objects.filter(project=request.auth.project, state=BotStates.JOINED_RECORDING)
            return Response(BotListSerializer(bots).data)
